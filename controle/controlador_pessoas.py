...
class ControladorPessoas():
  ...
  def registrar_compra(self):
    ...
    if(pessoa is not None):
      ...
      if self.__controlador_sistema.controle_carros_classicos.vender_carro(doc_carro, valor):
  ...
  def registrar_venda(self):
    ...
    if(pessoa is not None):
      # TRY EXCEPT
      carro = self.__controlador_sistema.controle_carros_classicos.pega_carro_por_vin(doc_carro)
      self.__controlador_sistema.controle_inspecao.fazer_inspecao(carro)

      pessoa.carros.del_carro(carro)

      doc_carro = self.__tela_pessoa.seleciona_carro()[0]
      valor = self.__tela_pessoa.seleciona_carro()[1]
      # APENAS SE LEN(VENDA) == LEN(COMPRA)
      self.__controlador_sistema.controle_carros_classicos.comprar_carro(doc_carro, valor)
